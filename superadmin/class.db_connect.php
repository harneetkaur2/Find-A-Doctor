...
class db {
	...
	function __construct($params=array()) {
	    ...
	    //connect('localhost', 'xorsolu_xorzyga', 'admin@12345', 'orsolu_xorzyga');
	...
	function getnum_rows($query) {
		// getnum_rows function: Get number of rows
		$cnx = $this->conn;
...
function getcalls_rows($query) { echo "fhghfg";
		// total company calls function: Get number of calls
		$cnx = $this->conn;
	
		$cur = @mysql_query($query, $cnx);
		
		$rows = @mysql_fetch_row($cur);
